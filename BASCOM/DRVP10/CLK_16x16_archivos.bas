...
'*******************************************************************************
Int_timer0:
   Timer0 = &H98                                            '600 Hz con 16MHz
   'Timer0 = &HD0                '600 con 7.3728MHZ
   'Timer0 = &H7E                                            '600 con 20 MHz
   Incr T0c
   T0c = T0c Mod 200
   If T0c = 0 Then
      Toggle led1
...
Sub Lee_enc()
   Varenc = Encoder(pind.5 , Pind.4 , Movizq , Movder , 0)
...
Sub Disptime()
   ...
   Tmpstr = Mid(tmptime , 1 , 1)                            'DecHora
   Tmpb = Val(tmpstr)
   Call Gendig(tmpb , 0)

   Tmpstr = Mid(tmptime , 2 , 1)                            'Unidades Hora
   Tmpb = Val(tmpstr)
   Call Gendig(tmpb , 1)

   Tmpstr = Mid(tmptime , 4 , 1)                            'Dec Minutos
   Tmpb = Val(tmpstr)
   Call Gendig(tmpb , 2)

   Tmpstr = Mid(tmptime , 5 , 1)                            'Unidades Minuto
   Tmpb = Val(tmpstr)
   Call Gendig(tmpb , 3)
